...
class PackageController extends Controller
{
    ...
    public function getPackages($id = '')
    {
        try
        {
            ...
            if($id != '')
            {
        	   $packageList = Package::select('package','available','videos','turnaround','overlays','music','fee','setup','name')
                                ->where('package', '=', $id)
                                ->get()->toArray();
            }
            else {
            ...
            {
                $responseData['packages'] = $packageList;
                $returnData = UtilityController::Generateresponse(1,'PACKAGE_LIST', Response::HTTP_OK, $responseData);
            }
            else {
